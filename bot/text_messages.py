from . import timeinkyiv_getter                                             #importing




start = "Slavcraft bot ввімкнений. Для допомоги у боті напишіть /help."     #texts

help = """Команди у боті:

/start - уввімкнути бота.
/help - допомога у боті.
/about - інформація про бота.
/credits - творці бота.
/rules - правила Славкрафту.
/ip - ip Славкрафту.
/members - лист усіх учасників СК.
/version - версія боту.
/flag - прапор Славкрафту.
/timeinkyiv - час у Києві.
/money - грошова система.


Також у деяких команд є підкоманди. Для допомоги с підкомандами напишіть /help_<назва команди>. Список команд у яких є підкоманди: /help, /ip, /flag, /members"""

help_help = """Підкоманди команди /help:

/help_help - підкоманди команди /help.
/help_ip - підкоманди команди /ip.
/help_flag - підкоманди команди /flag.
/help_timeinkyiv - підкоманди команди /timeinkyiv."""

help_ip = """Підкоманди команди /ip:

/ip_ip - ip як ip."""

help_flag = """Підкоманди команди /flag:

/flag_original - орігінальний файл прапора (іконка сервера)."""

help_timeinkyiv = """Підкоманди команди /timeinkyiv:

/timeinkyiv_seconds - час у києві з секундами."""

help_members = """Для того щоб вийшла підкоманда /members_<нікнейм граця актуальний для команди /members> - це вам виведе опис гравця написаний ним же самим. Також є список підкоманд /members:

/members_slavchik - опис гравця slavchik
/members_Savalio - опис гравця Savalio
/members_MarkoAntonio11 - опис гравця MarkoAntonio11
/members_11_ArtemPR_23 - опис гравця 11_ArtemPR_22
/members_mr_bacoun - опис гравця mr_bacoun
/members_bear0re0 - опис гравця bear0re"""


about = "Цей бот того шоб у учасників Славкрафту був швидкий доступ до інформації."


credits = "Цей бот створений slavchik'ом. Розробник інфраструктири – yngvarrs."


rules = """Правила Славкрафту:


Загальні

1.1. Неможно ображати інших учасників сервера.
1.2. Неможно спамити.
1.3. Неможно добавляти гравців на сервер без дозволу slavchik.
1.4. Заборонена проросійська/прорадянська діяльність.
1.5. Заборонена комуністична/соціалістична діяльність.
1.6. Заборонена нацистська/фашистська діяльність.
1.7. Заборонено росповшюджувати непристойний контент.
1.8. Ваш RP ні повинен нікому заважати.
1.9. Неможно файтитеся у чаті Майнкрафту, группі у Телеграмі та на сервері в Дискорде.

Майнкрафт

2.1. Неможно гриферити.
2.2. Неможно красти.
2.3. Неможно будувати лаг машини.
2.4. Неможно пвпшиться без дозволу всіх участников пвп.
2.5. Не чятрепортети/скінрепортети/нікрепортети інших гравців.
2.6. Неможно читерить на виживашках.

Умови для приєднання на СК

3.1. Кандидату повинно бути більше ніж 8 років.
3.2. Кандитат повинен знати українську мову та російську мову.
3.3. Потрібна ліцензія Minecraft Java Edition для приєднання.
3.4. Потрібно наявність Telegram або Discord.

creative-18

4.1. 25 блоків від координат 0, 0 у звичайному світі не можуть нікому належати.
4.2. 500 блоків від координат 0, 0 у пеклі не можуть нікому належати.
4.3. 250 блоків від координат 0, 0 в ендер світі не можуть нікому належати.

creative-20

5.1. 400 блоків від координат 0, 0 у звичайному світі не можуть нікому належати.
5.2. 25 блоків від координат 0, 0 у пеклі не можуть нікому належати.
5.3. 250 блоків від координат 0, 0 в ендер світі не можуть нікому належати."""


ip = """Ip Славкрафту:

creative-20: slavcraft.somerandom.xyz
?: slavcraft.somerandom.xyz:25566
creative-18: slavcraft.somerandom.xyz:25567"""

ip_ip = """Ip Славкрафту як ip:

creative-20: 141.95.73.17 
?: 141.95.73.17:25566
creative-18: 141.95.73.17:25567"""


members = """Перелік всіх учасників Славкрафту:

1. slavchik
2. TEBSS
3. Savalio
4. salad1210
5. MarkoAntonio11
6. joker497535
7. NancyCat1
8. 11_ArtemPR_23
9. Pla3ma
10. mr_bacoun
11. Matviy_krpk
12. YourLocalFurry0
13. bear0re0
14. WonderAir165649"""

members_slavchik = """Один з засновників Славкрафту, головний адмін СК. Грає у Minecraft, Kerbal Space Program, Oxygen Not Included, Portal 2, Garry's mod та браузерні ігри. Пройшов Minecraft, Portal, Portal 2, Portal 2 coop, Aprture Tag: The Paint Gun Testing Initiative, Portal Stories: Mel, Half-life, Half-life 2, Half life 2: Lost Coast, Half life 2: Episode One, Half life 2: Episode Two, Subnautica. Созасновник телеграм каналу "Меми та й не тільки!". Засновник та головний адмін телеграм групи Союз Minecraft. Має телеграм канал: Приключения Славы (https://t.me/@Priklyucheniya_Slavy) та ютуб канал: Приключения Славы (https://youtube.com/@Priklyucheniya_Slavy). Нікнейм у майнкрафті: slavchik. Телеграм псевдонім @Ukranianball. Діскорд ім'я: slavchik__."""

members_TEBSS = None

members_Savalio = "Ну я синяя морковка"

members_salad1210 = None

members_MarkoAntonio11 = "пон"

members_joker497535 = None

members_NancyCat1 = None

members_11_ArtemPR_23 = "Человек вроде обычный, а вроде и нет."

members_Pla3ma = None

members_mr_bacoun = "я бекон и я вас убю (еси меня ударить)"

members_Matviy_krpk = None

members_YourLocalFurry0 = None

members_bear0re0 = "я люблю ебатся с майяй"

members_WonderAir165649 = None


version = "Версія бота: розробка 0.2"


flag = "Прапор Славкафту:"

flag_original = "Орігінальний прапор Славкрафту (іконка сервера):"


def timeinkyiv():
    return f"Час у Києві: {timeinkyiv_getter.get_time()['h']}:{timeinkyiv_getter.get_time()['m']}"


def timeinkyiv_seconds():
    return f"Час у Києві: {timeinkyiv_getter.get_time()['h']}:{timeinkyiv_getter.get_time()['m']}:{timeinkyiv_getter.get_time()['s']}"
